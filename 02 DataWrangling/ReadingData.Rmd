...
The column names are human readable but not very handy for coding. **Indeed, in general, only dots and underscores should be used in variable names.** There is a quick way to solve this.
...
R has functions for reading in text files. However, the functions provided by tidyverse are more powerful. When reading in data, R reports on the column types. Information about the different types and their labels can be found here: [Column Types](https://tibble.tidyverse.org/articles/types.html)
